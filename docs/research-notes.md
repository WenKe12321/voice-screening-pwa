...
1. 使用 EATD-Corpus 建立中文开放回答基线。
2. 使用 Androids Corpus 单独评估《北风与太阳》朗读任务。
3. 完成 MODMA EULA 并取得授权后，冻结模型进行中文临床外部验证。
4. 独立报告 F1、召回率、特异度、ROC-AUC、PR-AUC、Brier 分数和置信区间。
5. 只有受试者隔离验证集达到门槛时，才导出可导入 `.vmodel`。
