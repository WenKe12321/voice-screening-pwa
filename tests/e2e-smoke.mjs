...
try {
  ...
  await page.getByLabel('地点').fill('学生服务中心 302')
  await page.getByLabel('开放时间').fill('周一至周五 9:00-17:00')
  await page.getByLabel('网页链接').fill('https://example.edu/counseling')
  await page.getByLabel('备注').fill('E2E 测试联系方式，不代表真实学校信息。')
  await page.getByRole('button', { name: '保存校内联系方式' }).click()
  await page.getByText('校内联系方式已保存到当前浏览器').waitFor()
  await page.getByText('测试大学心理健康教育中心').waitFor()
